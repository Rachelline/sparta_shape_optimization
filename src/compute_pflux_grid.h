...
ComputeStyle(pflux/grid,ComputePFluxGrid)

#else

#ifndef SPARTA_COMPUTE_PFLUX_GRID_H
#define SPARTA_COMPUTE_PFLUX_GRID_H

#include "compute.h"

namespace SPARTA_NS {

class ComputePFluxGrid : public Compute {
 public:
  ComputePFluxGrid(class SPARTA *, int, char **);
  ~ComputePFluxGrid();
  void init();
  void compute_per_grid();
  int query_tally_grid(int, sfloat **&, int *&);
  void post_process_grid(int, int, sfloat **, int *, sfloat *, int);
  void reallocate();
  bigint memory_usage();
